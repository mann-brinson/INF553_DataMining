...
def construct_data(filename, filter_thresh):
	user_busset_filter = sc.textFile(filename)\
...
def create_1item_bm(k, user_bus, cand_bitmap_full):
	...
	cand_bitmap = user_bus.map(lambda x: ord(x[0][0]))\
...
def create_1item_candidates(k, user_bus, cand_bitmap_full, cand_itemsets_full):
	...
	cand_itemset_rdd = user_bus.map(lambda x: (x[0], hash(ord(x[0][0]), num_buckets)))\
...
start = time.time()
user_busset_filter = construct_data(input_file_path, filter_thresh)
user_bus = user_busset_filter.flatMap(lambda x: [(bus, [x[0], 1]) for bus in x[1]])
...
for k in range(1, max_k):
	...
	if k > 1 and k < max_k:
		...
		#Elseif candidates found
			...
			#Make a pass through baskets, scanning for candidates. TIME CONSUMING
			poss_cands = user_busset_filter.flatMap(lambda x: check_basket(x[1], poss_cand_itemsets)).persist()
			...
			poss_cands_hash = poss_cands.map(lambda x: (hash(tb_hashed_str(x[0]), num_buckets), 1))\
			.reduceByKey(lambda x, y: x + y)\
			.map(lambda x: (x[0], 1) if x[1] >= support else (x[0], 0))
			...
			most_likely_cands = poss_cands.filter(lambda x: cand_bitmap_full.get(k).get(hash(tb_hashed_str(x[0]), num_buckets)) == 1)\
